  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setContracts(prev => (prev.length > 0 ? prev : (data || [])));
      }
    } catch (err) {
  ...
  const handleRefreshAndSave = async () => {
    ...
    try {
      ...
      if (res.success && res.list && res.list.length > 0) {
        ...
        showToast(`Successfully refreshed live API! ${res.list.length} contracts updated in database.`, "success");
      } else {
        // API failed or returned 0 items -> preserve old data
        if (res.list && res.list.length > 0) {
          setContracts(res.list);
        }
        showToast(res.error || "Could not retrieve new contracts from SAM.gov API. Retaining existing contracts.", "warning");
      }
    } catch (err) {
      console.error("Sync error:", err);
      showToast("API sync notice: Could not fetch contracts from SAM.gov API. Retaining existing data.", "warning");
    } finally {
      setSyncing(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200/60">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Connect to SAM.gov API to fetch, update, and persist federal courier contracts in the database
          </p>
        </div>
        <button
          onClick={handleRefreshAndSave}
          disabled={syncing}
          className="px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-bold transition-all shadow-2xs cursor-pointer flex items-center gap-2 self-start sm:self-auto disabled:opacity-50"
        >
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-lg overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-extrabold text-[#0b132b] font-serif-heading">Active Government Contracts</h3>
            <p className="text-xs text-slate-400 font-medium">Federal courier, logistics, & medical transport solicitations</p>
          </div>
